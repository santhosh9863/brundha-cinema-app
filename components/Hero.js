  ...
  return (
    <group ref={groupRef} position={position}>
      <Sphere args={[1, 48, 48]} ref={ref}>
        <meshStandardMaterial color={color} transparent opacity={opacity} roughness={0.4} metalness={0.95} emissive={glow} emissiveIntensity={0.6} />
      </Sphere>
      <Sphere args={[1.3, 32, 32]}>
        <meshStandardMaterial color={glow} transparent opacity={opacity * 0.2} emissive={glow} emissiveIntensity={1} />
      </Sphere>
    </group>
  );
}
...
function MouseCamera() {
  ...
  useEffect(() => {
    const handleMove = (e) => {
      ...
      const cy = window.innerHeight / 2;
      target.current = {
        x: ((e.clientX - cx) / cx) * 0.4,
        y: 0.15 + ((e.clientY - cy) / cy) * 0.2,
      };
    };
    window.addEventListener("mousemove", handleMove, { passive: true });
...
function ParticleField() {
  ...
  return (
    <points ref={ref}>
      ...
      <pointsMaterial size={0.025} color="#ffd700" transparent opacity={0.5} sizeAttenuation blending={THREE.AdditiveBlending} />
    </points>
  );
}
...
function CTAButton({ href, children, variant = "primary" }) {
  ...
  const base = "relative overflow-hidden px-8 sm:px-10 py-3.5 sm:py-4 text-sm sm:text-base rounded-full font-semibold transition-shadow duration-300";

  const variants = {
    primary: "bg-gradient-to-r from-yellow-400 to-amber-500 text-black shadow-[0_0_20px_rgba(255,200,0,0.3)] hover:shadow-[0_0_40px_rgba(255,200,0,0.5)]",
    secondary: "border border-white/20 text-white/80 hover:text-white hover:border-yellow-400/50 hover:bg-yellow-400/5 backdrop-blur-sm",
  };
    ...
    >
      ...
      {ripples.map((r) => (
        <motion.span
          ...
          style={{ left: r.x, top: r.y, width: 30, height: 30, marginLeft: -15, marginTop: -15 }}
        />
      ))}
      {variant === "primary" && (
        <motion.div
...
const fadeUp = {
  ...
  visible: (i = 0) => ({
    ...
    y: 0,
    filter: "blur(0px)",
    transition: { duration: 0.8, ease: [0.25, 0.46, 0.45, 0.94], delay: i * 0.12 },
  }),
};
...
export default function Hero() {
  ...
  const bgLayer = useParallax(8);
  const midLayer = useParallax(16);
  const fgLayer = useParallax(24);
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden text-white w-full bg-black">
      {/* 3D Canvas Layer */}
      <div ref={bgLayer} className="absolute inset-0 z-0 will-change-transform">
        <Canvas camera={{ fov: 50, position: [0, 0.15, 4.2] }} dpr={[1, 1.5]} gl={{ antialias: true, alpha: true }}>
          <Scene />
        </Canvas>
      </div>
      ...
      {/* Floating Glow Blobs — Mid Layer */}
      <div ref={midLayer} className="absolute inset-0 z-10 pointer-events-none will-change-transform">
        <div className="absolute top-[15%] left-[10%] w-72 h-72 bg-yellow-400/8 rounded-full blur-[120px]" />
        <div className="absolute bottom-[20%] right-[15%] w-96 h-96 bg-purple-500/6 rounded-full blur-[140px]" />
        <div className="absolute top-[60%] left-[50%] w-64 h-64 bg-amber-500/5 rounded-full blur-[100px]" />
      </div>
      ...
      {/* Content Layer — Foreground Parallax */}
      <div ref={fgLayer} className="relative z-30 will-change-transform">
        <motion.div
          className="flex flex-col items-center justify-center text-center px-4 sm:px-6 max-w-4xl"
        ...
        >
          ...
          {/* Title */}
          <motion.h1 variants={fadeUp} custom={1} className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold leading-[0.9] tracking-tight">
            <span className="block text-white">BRUNDHA</span>
          ...
          {/* Divider */}
          <motion.div variants={fadeUp} custom={2} className="mt-6 sm:mt-8 w-16 sm:w-24 h-[1px] bg-gradient-to-r from-transparent via-yellow-400/60 to-transparent" />

          {/* Subtitle */}
          <motion.p variants={fadeUp} custom={3} className="mt-6 sm:mt-8 text-base sm:text-lg md:text-xl text-white/60 max-w-xl leading-relaxed font-light">
          ...
          {/* Feature Pills */}
          <motion.div variants={fadeUp} custom={4} className="flex flex-wrap justify-center gap-2 sm:gap-3 mt-6 sm:mt-8">
            {["Dolby Atmos", "4K Laser", "Recliner Seats", "IMAX Sound"].map((f) => (
              <motion.span
                key={f}
                whileHover={{ scale: 1.08, borderColor: "rgba(255,215,0,0.3)" }}
                className="px-3 py-1.5 rounded-full border border-white/10 bg-white/5 text-white/50 text-xs sm:text-sm backdrop-blur-sm cursor-default transition-colors"
              >
                {f}
              </motion.span>
            ))}
          ...
          {/* Buttons */}
          <motion.div variants={fadeUp} custom={5} className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-8 sm:mt-10 w-full sm:w-auto">
            <CTAButton href="/booking" variant="primary">Book Tickets</CTAButton>
            <CTAButton href="#features" variant="secondary">Explore More</CTAButton>
          </motion.div>

          {/* Scroll Indicator */}
          <motion.div variants={fadeUp} custom={6} className="mt-12 sm:mt-16 flex flex-col items-center gap-2">
